...
import ppc.frame.annotation.ManagerPriority;
import ppc.frame.manager.FileManager;
import ppc.frame.manager.Manager;
...
public class Launch {
	...
	private static void init() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
		...
		Reflections reflections = new Reflections("ppc.frame");
		Set<Class<?>> types = reflections.getTypesAnnotatedWith(ppc.frame.annotation.Manager.class);
		criticalPrioManager.addAll(types.stream().filter(manager -> manager
				.getAnnotation(ppc.frame.annotation.Manager.class).priority() == ManagerPriority.CRITICAL).toList());

		highPrioManager.addAll(types.stream().filter(
				manager -> manager.getAnnotation(ppc.frame.annotation.Manager.class).priority() == ManagerPriority.HIGH)
				.toList());

		mediumPrioManager.addAll(types.stream().filter(manager -> manager
				.getAnnotation(ppc.frame.annotation.Manager.class).priority() == ManagerPriority.MEDIUM).toList());

		lowPrioManager.addAll(types.stream().filter(
				manager -> manager.getAnnotation(ppc.frame.annotation.Manager.class).priority() == ManagerPriority.LOW)
				.toList());
